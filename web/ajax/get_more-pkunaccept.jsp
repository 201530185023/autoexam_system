...
<p>
  ...
  考卷挑战 <button class="btn btn-sm btn-success">接受</button> <button class="btn btn-sm btn-danger">拒绝</button>
</p><%}else{%>
<p>
  ...
  考卷挑战 <button role="btn-cancelunaccept"
               data-name="{'paperID':'<%=pkHistory.getPaper_id()%>','user':'<%=pkHistory.getUser2()%>'}" class="btn btn-sm btn-danger">取消</button>
</p>
<%}%>
<%}%>